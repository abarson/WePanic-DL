...
def get_module_attributes(module, exclude_set=None):
    """
    get the programmed attributes of a module, not the 
    __#__ flavor terms or any of the builtin items, just
    those generated by the programmer

    args:
        module: an imported module
        excluxe_set: (list-like) - the set of not include in the attrs
    returns:
        (list (str)) : a list of strings symbolizing the model attributes
    """
    if exclude_set is None:
        exclude_set = set()

    return list(filter(lambda k: k[:2] != '__' and k[-2:] != '__' and k not in exclude_set, vars(module).keys()))
